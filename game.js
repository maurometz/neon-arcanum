document.addEventListener('DOMContentLoaded', () => {
    ...
    function resetPlayerStats() {
        player = {
            ...
            health: 100, maxHealth: 100, moveSpeed: 180, firewall: 0,
    ...
    const weaponDefinitions = {
        ...
        ferroadaAlgoritmo: { name: 'Ferroada de Algoritmo', description: 'Projéteis teleguiados que perfuram até dois inimigos.', baseCooldown: 1, attack: (p, weapon) => { const target = findNearestEnemy(p.x, p.y); if (!target) return; const angle = Math.atan2(target.y - p.y, target.x - p.x); const speed = 300 * p.resolucao; projectiles.push({ type: 'homing_piercing', x: p.x, y: p.y, radius: 6, color: '#00e5e5', target: target, speed: speed, dx: Math.cos(angle) * speed, dy: Math.sin(angle) * speed, damage: 12 * p.codificacao, lifetime: 240, source: 'ferroadaAlgoritmo', pierceLeft: 2 + weapon.level, turnSpeed: 0.1, hitEnemies: [] }); } },
    ...
    function updatePlayer(deltaTime) {
        ...
        if (dx !== 0 || dy !== 0) { const length = Math.sqrt(dx * dx + dy * dy); player.x += (dx / length) * currentMoveSpeed * deltaTime; player.y += (dy / length) * currentMoveSpeed * deltaTime; }
    ...
    function spawnEnemy() { const side = Math.floor(Math.random() * 4); let x, y; if (side === 0) { x = Math.random() * canvas.width; y = -ENEMY_SPRITE_SIZE; } else if (side === 1) { x = canvas.width + ENEMY_SPRITE_SIZE; y = Math.random() * canvas.height; } else if (side === 2) { x = Math.random() * canvas.width; y = canvas.height + ENEMY_SPRITE_SIZE; } else { x = -ENEMY_SPRITE_SIZE; y = Math.random() * canvas.height; } const randomSprite = enemySprites[Math.floor(Math.random() * enemySprites.length)]; enemies.push({ x, y, radius: 28, sprite: randomSprite, speed: 72, health: 8 + (difficultyTimer / 8), damage: 10, slowUntil: 0, confusionUntil: 0 }); }
    ...
    function updateEntities(deltaTime) {
        if (gamePaused) return;
        enemies.forEach((enemy, index) => { if (enemy.health <= 0) { handleEnemyDeath(enemy, index); return; } let targetX = player.x, targetY = player.y; let currentSpeed = enemy.speed; if (gameTime < enemy.confusionUntil) { if (!enemy.randomDir) enemy.randomDir = { x: (Math.random() - 0.5) * 2, y: (Math.random() - 0.5) * 2 }; enemy.x += enemy.randomDir.x * currentSpeed * deltaTime; enemy.y += enemy.randomDir.y * currentSpeed * deltaTime; } else { enemy.randomDir = null; const dx = targetX - enemy.x, dy = targetY - enemy.y; const distance = Math.hypot(dx, dy); if (gameTime < enemy.slowUntil) currentSpeed *= 0.5; if (distance > 1) { enemy.x += (dx / distance) * currentSpeed * deltaTime; enemy.y += (dy / distance) * currentSpeed * deltaTime; } } const playerDist = Math.hypot(player.x - enemy.x, player.y - enemy.y); if (playerDist < player.radius + enemy.radius) { dealDamage(player, enemy.damage, 'collision'); enemies.splice(index, 1); } });
        ...
        damageNumbers.forEach((dn, index) => { dn.y -= 60 * deltaTime; dn.lifetime--; dn.alpha = dn.lifetime / 60; if (dn.lifetime <= 0) damageNumbers.splice(index, 1); });
    }
